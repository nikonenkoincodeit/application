      ...
      <q-card-section class="col-lg-4 col-md-6">
         ...
         <q-form class="signup-form">
          ...
          />
            ...
            />
            <q-btn label="Next" type="button" class="btn-next" @click="onNextStep"/>
          </div>
...
export default {
  ...
  methods: {
    ...
    onNextStep () {
      ...
      if (!this.$v.lastName.$error && !this.$v.userEmail.$error && !this.$v.firstName.$error) {
        this.step = 1;
      }
    },
    createAccount () {
      ...
      if (!this.$v.userName.$error && !this.$v.password.$error && !this.$v.repeatPassword.$error) {
        this.register({
